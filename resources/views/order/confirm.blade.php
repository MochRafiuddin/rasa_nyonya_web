            ...
            <div class="card-body">
                <h6 class="card-title">{{$titlePage}} Order</h6>                
                    ...
                    <div class="d-flex justify-content-start">
                        <button class="btn btn-success mr-2 status-btn" data-status="4">Accept</button>
                        <button class="btn btn-danger status-btn" data-status="5">Reject</button>
                    </div>                
            </div>
        </div>
    </div>
        ...
        <div class="modal fade" id="modalInput" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel-3" aria-hidden="false">
          <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
              ...
              <form method="post" action="{{url('order/update-status')}}">
                @csrf
                  <div class="modal-body">
                  <input type="hidden" name="id_order" value="{{$data->id_order}}" id="id_order">
                  <input type="hidden" name="id_status_m" value="" id="id_status_m">
                      <div class="fee">
                          <label>Fee</label>
                          <input type="text" class="form-control numeric" name="fee_courier" value="{{$fee->fee}}">
                      </div>
                      <div class="alasan_reject">
                          <label>Alasan Reject</label>
                          <textarea class="form-control" cols="5" rows="6" value=""
                        name="alasan_reject"></textarea>
                      </div>
                  </div>
...
<script>        
    ...
    $('body').on('click', '.status-btn', function () {        
        ...
        if (status == 4) {            
            $(".fee").show();
            $(".alasan_reject").hide();
            $("#exampleModalLabel-3").html('Accept');
            $("#id_status_m").val('4');
        }else{
            $(".fee").hide();
            $(".alasan_reject").show();
            $("#exampleModalLabel-3").html('Reject');
            $("#id_status_m").val("5");
        }        
